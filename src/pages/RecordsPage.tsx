        ...
        <div className="space-y-2">
          ...
          {customFields.map((field, index) => (
            <div key={index} className="flex gap-2">
              <Input
                placeholder="Field name (PAN, GSTIN...)"
                value={field.key}
                onChange={(e) => {
...
export default function RecordsPage() {
  ...
  const handleCreate = async (
    data: Omit<Record, "id" | "created_at" | "status">,
  ) => {
    setSaving(true);
    try {
  ...
  const filtered = enriched.filter((r) => {
    ...
    const matchService =
      filterService === "all" || r.service_type === filterService;
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <Card
            key={stat.label}
            className={cn("p-4 rounded-2xl border shadow-sm", stat.bg)}
          >
      ...
      <div className="rounded-2xl border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-900/20 px-5 py-3 flex items-center gap-3">
        ...
        <p className="text-xs text-blue-700 dark:text-blue-300">
          <strong>Auto Reminder System:</strong> WhatsApp messages are sent
          daily at 9:00 AM for clients whose due date matches their reminder
          schedule.
        </p>
      </div>
        ...
        {loading ? (
          ...
          <div className="p-12 text-center space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              {records.length === 0
                ? "No records yet. Add your first client record."
                ...
                {filtered.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <TableCell>
                        <ServiceBadge type={r.service_type} />
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1 text-xs text-muted-foreground">
                          <Clock className="h-3 w-3" /> {r.reminder_days_before}{" "}
                          days before
                        </div>
                      </TableCell>
                      <TableCell>
